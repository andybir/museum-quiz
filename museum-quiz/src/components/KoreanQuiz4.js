...
class KoreanQuiz4 extends Component {
    constructor(props) {
        super(props)
        this.state = {
            quiz: this.props.quiz.questions[13]
        }
    }
    ...
    getAnswer = (value) => {
        console.log(value)
        if (value) {
            this.props.history.push('/korean-correct4')
        } else {
            this.props.history.push('/korean-incorrect4')
        }
        // Modal???    
        } 
    ...
    render () {
        // this.getAnswer()
        const quiz = this.props.quiz.questions[13] 
        ...
        return(
            <div className='quiz'>
                ...
                <button><Link to={'/korean-art4'}>Go Back</Link></button>
...
export default withRouter(KoreanQuiz4)